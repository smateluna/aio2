...
public class SolicitudServiceAction extends CbrsAbstractAction {

	private static final Logger logger = Logger.getLogger(SolicitudServiceAction.class);
	...
	public void saveSingleConValidacion(ActionMapping mapping, ActionForm form,
		...
		if(StringUtils.isBlank(usuarioCreador)){
			...
			try {
				...
				ConsultaDocumentoDTO consultaDocumentoDTO = digitalUtil.getConsultaDocumentoDTO(foja, numero, ano, bisInscripcion, 1);
				...
				if(consultaDocumentoDTO!=null){  //tengo respuesta?				
					...
					if(consultaDocumentoDTO.getHayDocumento()){ //tengo papel?
						...
						if(fna){
							...
							Boolean resultado = solicitudUtil.solicitar(usuarioCreador, foja, numero, ano, bis);
