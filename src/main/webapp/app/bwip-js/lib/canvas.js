...
(function() {
	...
	for (var i = 0; i < symdesc.length; i++) {
		if (!symdesc[i])	// last entry can be null
			continue;
		emit('bwip-js/bwipp/' + symdesc[i].sym);
	}

	// Renderers
	emit('bwip-js/bwipp/renlinear');
	emit('bwip-js/bwipp/renmatrix');
	emit('bwip-js/bwipp/renmaximatrix');

	// Fonts
	//comentado para mejora de performance 12/02/2015 rgonzaleza
//	for (var x = 1; x <= 10; x++) {
//		emit('bwip-js/fonts/ocrb10-' + (x < 10 ? '0' : '') + x);
//		emit('bwip-js/fonts/ocrb12-' + (x < 10 ? '0' : '') + x);
//	}
