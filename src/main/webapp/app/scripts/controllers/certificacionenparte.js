...
app.controller('CertificacionEnParteCtrl', function ($scope,$timeout,$rootScope,$location,$anchorScroll,solicitudesModel,certificacionModel, certificacionService, caratulaService, $filter,$modal,$modalStack) {
	...
	var promise = certificacionService.obtenerCaratulasParaCertificarEnParte();
...
//		var promise = caratulaService.movercaratula(inscripcion.numerocaratula);
//		promise.then(function(data) {
			$scope.closeModal();
//		    if(data.status===null){
//		    }else if(data.status){
	    	var bis = inscripcion.bis==1?0:1;
				console.log('url '+'/verInscripcionCertificar/prop/'+inscripcion.numeroCaratula+'/'+inscripcion.foja+'/'+inscripcion.numero+'/'+inscripcion.ano+'/'+inscripcion.bis+'/'+inscripcion.fechaDocLong+'/'+inscripcion.idtipoFormulario+'/certificacionenparte/');
			    $rootScope.go('/verInscripcionCertificar/prop/'+inscripcion.numeroCaratula+'/'+inscripcion.foja+'/'+inscripcion.numero+'/'+inscripcion.ano+'/'+inscripcion.bis+'/'+inscripcion.fechaDocLong+'/'+inscripcion.rehaceImagen+'/10/certificacionenparte/');
//		    }else{
//		      $scope.setErr('data.msg', data.msg);
//		    }
//		  }, function(reason) {
...
//		});
	...
	$scope.verInscripcionCertificar = function(inscripcion){
	    var bis = inscripcion.bis==1?0:1;
		console.log('url '+'/verInscripcionCertificar/prop/'+inscripcion.caratula+'/'+inscripcion.foja+'/'+inscripcion.numero+'/'+inscripcion.ano+'/'+inscripcion.bis+'/'+inscripcion.fechaDocLong+'/'+inscripcion.idtipoFormulario+'/certificacionenparte/');
	    $rootScope.go('/verInscripcionCertificar/prop/'+inscripcion.caratula+'/'+inscripcion.foja+'/'+inscripcion.numero+'/'+inscripcion.ano+'/'+inscripcion.bis+'/'+inscripcion.fechaDocLong+'/'+inscripcion.rehaceImagen+'/'+inscripcion.idtipoFormulario+'/certificacionenparte/');
    }; 
