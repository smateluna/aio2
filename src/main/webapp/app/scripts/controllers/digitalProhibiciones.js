...
app.controller('DigitalProhibicionesCtrl', function ($rootScope, $scope, $timeout, $location, digitalProhibicionesModel, $filter, $modal,
                                        $modalStack, inscripcionDigitalProhibicionesService, solicitudProhibicionesService) {
  //modelos
  $scope.tab = digitalProhibicionesModel.getTab();
  $scope.busquedaTitulo = digitalProhibicionesModel.getBusquedaTitulo();
  $scope.busquedaMis = digitalProhibicionesModel.getBusquedaMis();
  $scope.states = digitalProhibicionesModel.getStates();
  ...
  $scope.activateParent = function(id){
    ...
    digitalProhibicionesModel.setTab($scope.tab);
  ...
  $scope.verTitulo = function(titulo){
    ...
    $rootScope.go('/verInscripcion/proh/'+titulo.foja+'/'+titulo.numero+'/'+titulo.ano+'/'+bis+'/'+$scope.solicita.origen);
  };
  ...
  $scope.buscarTitulo = function(){
    ...
    var promise = inscripcionDigitalProhibicionesService.getInscripcion(foja, numero, ano, bis);
  ...
  $scope.openSolicitar = function () {
    ...
    $modal.open({
      ...
      resolve: {
        ...
        servicio: function () {
          return solicitudProhibicionesService;
        },
        origen: function(){
  ...
  $scope.buscarMis = function(loading){
    ...
    var promise = solicitudProhibicionesService.getByUserInSession();
    promise.then(function(data) {
      if(loading){
        $scope.closeModal();
      }
  ...
  $scope.accionSolicitud = function(promesa, solicitud){
    ...
    if(solicitud.estadoDTO.idEstado==5){
      ...
      $scope[promesa] = solicitudProhibicionesService.actualizar(solicitud.idSolicitud, accion);
