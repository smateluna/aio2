...
app.controller('DigitalCtrl', function ($rootScope, $scope, $timeout, $location, digitalModel, $filter, $modal,
	$modalStack, inscripcionDigitalService, solicitudService) {
	//modelos
	$scope.tab = digitalModel.getTab();
	$scope.busquedaTitulo = digitalModel.getBusquedaTitulo();
	$scope.busquedaMis = digitalModel.getBusquedaMis();
	$scope.states = digitalModel.getStates();
	...
	$scope.verTitulo = function(titulo){
		...
		$rootScope.go('/verInscripcion/prop/'+titulo.foja+'/'+titulo.numero+'/'+titulo.ano+'/'+bis+'/digital');
	};
