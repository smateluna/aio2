...
app.controller('VerInscripcionCertificarProhCtrl', function ($scope, $routeParams, $rootScope, $modal, $modalStack, $sce, $timeout,inscripcionDigitalProhibicionesService,caratulaService, certificacionProhibicionesService) {
	...
	$scope.buscar = function(){
		...
		$scope.urlPDF = $sce.trustAsResourceUrl("../do/service/inscripcionDigitalProhibiciones?metodo=verInscripcionCertificar&caratula="+caratula);
		$scope.isLoading  = false;


		var promise = inscripcionDigitalProhibicionesService.getInscripcion(
...
//			});
		...
		}else if((!tieneanotacion2 && $scope.data.estado.rehacerImagen) && $scope.data.inscripcionDigitalDTO.ano<$rootScope.aioParametros.anoDigitalProhibiciones){
			$scope.parametros.rehaceImagen='1';
		}else{
		...
		$modal.open({
			...
			resolve: {
				...
				servicioCertificacion: function(){
					return certificacionProhibicionesService;
				}
		}
		});
	};
	...
	$scope.certificar = function(){
		...
		var promise = certificacionProhibicionesService.certificar($scope.parametros.caratula,$scope.parametros.foja, $scope.parametros.numero, $scope.parametros.ano, $scope.parametros.bis,$scope.certificacion.tipo.codigo);
		promise.then(function(data) {
			$scope.closeModal();
			if(data.status===null){
	...
	$scope.distribuir = function(){
		...
		var promise = certificacionProhibicionesService.distribuir($scope.parametros.caratula,$scope.certificacion.tipo.codigo);
		promise.then(function(data) {
			$scope.closeModal();
			if(data.status===null){
	...
	$scope.loadWidgets = function(tipo) {
		...
		if (tipo === 'borradores') {
			promise = inscripcionDigitalProhibicionesService
